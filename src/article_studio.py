...
# # # # # def generate_draft(platform: str, topic: str, target_url: str = None) -> dict:
# # # # #     """
# # # # #     platform: 'contra' or 'notion'.
# # # # #     topic: what the post/article should be about.
# # # # #     target_url: required for contra (the thread being replied to), ignored for notion.
# # # # #     """
# # # # #     if not config.GEMINI_API_KEY:
# # # # #         return {"status": "failed", "detail": "GEMINI_API_KEY not set in .env."}

# # # # #     if platform == "contra" and not target_url:
# # # # #         return {"status": "failed", "detail": "Contra drafts need a target thread URL to reply to."}
...
# # # # #         else:  # contra
# # # # #             prompt = f"""
# # # # # Write a short community comment (2-4 sentences) replying to a thread about: {topic}
# # # # # This is for Gaper (gaper.io), to be posted as a real comment on Contra.
...
# # # # #             draft_id = create_article_draft(platform="contra", topic=topic, content=content, target_url=target_url)
...
# # # # #         adapter = get_adapter("contra")
# # # # #         result = adapter.execute_post(draft.target_url, draft.content)
# # # # #         # ContraAdapter already logs to PostedBacklink on success internally
...
# # # # #             # Browser-typed post bypasses NotionAdapter's own logging - log it
# # # # #             # here directly so it still shows up in the New Backlinks tab.
# # # # #             db = SessionLocal()
# # # # #             try:
...
# # # # #                     note="Typed via browser session - Publish to web still needs your manual click.",
# # # # #                 ))
# # # # #                 db.commit()
# # # # #             finally:
# # # # #                 db.close()
...
# # # # # src/article_studio.py
# # # # """
# # # # Article Studio - Generate, edit, improve, submit drafts for Contra & Notion
# # # # """
...
# # # # # Permanent Contra URL (from root post_to_contra.py)
# # # # CONTRA_FEED_URL = getattr(config, "CONTRA_FEED_URL", "https://contra.com/community/for-you")
...
# # # # def _case_study_block(query_text: str) -> str:
# # # #     cases = retrieve_relevant_case_studies(query_text)
# # # #     return "\n".join(f"- {c}" for c in cases) if cases else "No specific case study matched."


# # # # def generate_draft(platform: str, topic: str, target_url: str = None) -> dict:
# # # #     """Generate a new draft using Gemini."""
# # # #     if not config.GEMINI_API_KEY:
# # # #         return {"status": "failed", "detail": "GEMINI_API_KEY not set in .env."}
...
# # # # Brand USPs:
# # # # {usps}
# # # # Case studies/facts you MAY cite naturally:
# # # # {case_block}
# # # # Link to include naturally once, INSIDE THE BODY ONLY: {config.PRIMARY_URL}
...
# # # #         else:  # contra
# # # #             # Same prompt as root/post_to_contra.py
# # # #             prompt = f"""
...
# # # #             contra_url = target_url or CONTRA_FEED_URL
            
# # # #             draft_id = create_article_draft(
# # # #                 platform="contra",
# # # #                 topic=topic,
# # # #                 content=content,
# # # #                 target_url=contra_url,
# # # #                 title=topic
# # # #             )
# # # #             return {"status": "success", "draft_id": draft_id, "content": content, "target_url": contra_url}
...
# # # # Revise to address the feedback. Keep the same format as before.
...
# # # # def submit_draft(draft_id: int) -> dict:
# # # #     """Submit draft LIVE to platform."""
# # # #     draft = get_article_draft(draft_id)
# # # #     if not draft:
# # # #         return {"status": "failed", "detail": "Draft not found."}

# # # #     if draft.platform == "contra":
# # # #         target_url = draft.target_url or CONTRA_FEED_URL
# # # #         result = post_to_contra_feed(draft.content, target_url)
...
# # # #                 db.add(PostedBacklink(
# # # #                     platform="notion",
# # # #                     target_url=getattr(config, "NOTION_DATABASE_URL", ""),
# # # #                     content=f"{draft.title}\n\n{draft.content}",
# # # #                     status="live",
# # # #                     note="Typed via browser session - Publish to web needs manual click.",
# # # #                 ))
# # # #                 db.commit()
# # # #             finally:
# # # #                 db.close()
...
# # # # # src/article_studio.py - Updated generate_draft with auto-topic
...
# # # #     "How to deploy AI agents in existing workflows",
...
# # # #     "AI-native implementation partner",
# # # #     "Deploying AI agents without breaking things",
...
# # # # def generate_auto_topic() -> str:
# # # #     """Generate a random topic for Contra post."""
# # # #     return random.choice(TOPICS)

# # # # def generate_draft(platform: str, topic: str = None, target_url: str = None) -> dict:
# # # #     """Generate a new draft using Gemini. If no topic given, auto-generate."""
...
# # # # src/article_studio.py
# # # """
# # # Article Studio - Uses root post_to_contra.py directly
# # # """

# # # import logging
# # # import random
# # # # src/article_studio.py - Add this at top
# # # import re  # Add this
# # # import google.generativeai as genai
# # # from src.database import (
# # #     create_article_draft, get_article_draft, update_article_draft,
# # #     delete_article_draft, PostedBacklink, SessionLocal
# # # )
# # # from src.gaper_scraper import get_brand_profile
# # # from src.notion_session_poster import post_to_notion_session
# # # import config

# # # # IMPORT the working root file directly
# # # import sys
# # # from pathlib import Path
# # # sys.path.insert(0, str(Path(__file__).parent.parent))
# # # from post_to_contra import post_to_contra, generate_post_content

# # # logger = logging.getLogger(__name__)
# # # GEMINI_MODEL = "gemini-flash-latest"

# # # # Auto-topics - will generate new content every time
# # # TOPICS = [
# # #     "AI agents in production",
# # #     "How to deploy AI agents in existing workflows",
# # #     "AI automation for support teams",
...
# # # # def generate_draft(platform: str, topic: str = None, target_url: str = None) -> dict:
# # # #     """Generate a new draft. Auto-generates topic if not provided."""
...
# # # #         else:  # contra
# # # #             # Use the SAME generator as root post_to_contra.py
# # # #             content = generate_post_content()  # This uses the working generator!
            
# # # #             contra_url = target_url or getattr(config, "CONTRA_FEED_URL", "https://contra.com/community/for-you")
...
# # # # In src/article_studio.py - Update generate_draft for Notion
...
# # # IMPORTANT RULES:
# # # - Line 1: ONLY the plain title text. NO links, NO markdown, NO brackets, under 70 characters.
# # # - The link https://gaper.io MUST be in the BODY of the article, NEVER in the title.
# # # - Use "## " for headings, "* " for bullets, "**text**" for bold.
# # # - Genuinely useful, factual tone, not a sales pitch.
# # # - Output ONLY the title + article, nothing else.
# # # """
...
# # #             # Clean title - remove any links
# # #             raw_title = lines[0].strip() if lines else topic[:70]
...
# # #                 title=clean_title
# # #             )
# # #             return {"status": "success", "draft_id": draft_id, "title": clean_title, "content": body}

# # #         else:  # contra
# # #             # ... existing contra code
...
# # # # def submit_draft(draft_id: int) -> dict:
# # # #     """Submit draft LIVE - uses root post_to_contra for Contra."""
# # # #     draft = get_article_draft(draft_id)
# # # #     if not draft:
# # # #         return {"status": "failed", "detail": "Draft not found."}

# # # #     if draft.platform == "contra":
# # # #         # Use the WORKING root post_to_contra function directly!
# # # #         result = post_to_contra(draft.content)
...
# # # #                 db.add(PostedBacklink(
# # # #                     platform="contra",
# # # #                     target_url=getattr(config, "CONTRA_FEED_URL", "https://contra.com/community/for-you"),
# # # #                     content=draft.content,
# # # #                     status="live",
# # # #                     note="Posted via root post_to_contra.py",
# # # #                 ))
# # # #                 db.commit()
# # # #             finally:
# # # #                 db.close()
...
# # # #                     content=f"{draft.title}\n\n{draft.content}",
# # # #                     status="live",
# # # #                     note="Posted via Notion session",
# # # #                 ))
# # # #                 db.commit()
# # # #             finally:
# # # #                 db.close()
...
# # # # src/article_studio.py - Update submit_draft function
...
# # #     elif draft.platform == "notion":
# # #         # Use the browser session to post
# # #         result = post_to_notion_session(draft.title or draft.topic, draft.content)
...
# # #                     content=f"{draft.title}\n\n{draft.content}",
# # #                     status="live",
# # #                     note="Posted via Notion browser session",
# # #                 ))
# # #                 db.commit()
# # #             finally:
# # #                 db.close()
...
# #                         <option value="contra">Contra (reply to a thread)</option>
...
# #             container.innerHTML = '';
# #             const active = drafts.filter(d => d.status === 'draft' || d.status === 'failed');
...
# #                             <span class="badge ${d.status === 'failed' ? 'badge-failed' : 'badge-pending'}">${d.status}</span>
# #                         </div>
# #                         ${d.target_url ? `<div style="font-size:12px;color:var(--text-muted);">Target: ${d.target_url}</div>` : ''}
# #                         ${d.detail ? `<div style="font-size:11px;color:var(--warning);">${d.detail}</div>` : ''}
# #                         <textarea id="draft-content-${d.id}" rows="6" style="width:100%;background:rgba(0,0,0,0.4);border:1px solid var(--glass-border);border-radius:8px;padding:12px;color:#fff;margin-top:10px;">${d.content || ''}</textarea>
...
# #                         <div class="meta-info"><span>Posted: ${new Date(b.created_at).toLocaleString()}</span></div>
# #                         <div class="actions-bar">
# #                             ${b.status !== 'deleted' ? `
...
# #             opps.sort((a, b) => (b.relevance_score || 0) - (a.relevance_score || 0));
# #             opps.forEach(o => {
# #                 const statusClass = o.status === 'listed' ? 'badge-listed' : o.status === 'failed' ? 'badge-failed' : 'badge-pending';
# #                 const statusLabel = o.status === 'listed' ? '✅ Listed' : o.status === 'failed' ? '❌ Failed' : o.status === 'pending_listing' ? '📝 Form Filled - Review' : '⏳ Discovered';
...
# #                             ` : o.status !== 'listed' ? `<button class="btn btn-sm" onclick="listGaper(${o.id})">🚀 Auto-fill Form</button>` : `<span style="color:var(--success)">✅ Already Listed</span>`}
# #                         </div>
# #                     </div>
# #                 `;
# #             });
...
#     """
#     Pulls the 32-char page id off the end of a notion.so URL and formats it
#     with dashes (8-4-4-4-12) the way the Notion API expects it, so live
#     edit/delete (notion_backlink_manager.py) can actually find the page.
#     """
#     if not url:
#         return None
...
# def submit_draft(draft_id: int) -> dict:
#     """Submit draft LIVE to platform, using the same saved sessions Contra/Notion already use."""
#     draft = get_article_draft(draft_id)
#     if not draft:
#         return {"status": "failed", "detail": "Draft not found."}

#     if draft.platform == "contra":
#         # Post to whatever target_url was set at generation time (feed or a
#         # specific profile/thread page) - falls back to the feed if none was given.
#         result = post_to_contra_feed(draft.content, draft.target_url)
...
#                 db.add(PostedBacklink(
#                     platform="contra",
#                     target_url=draft.target_url or getattr(config, "CONTRA_FEED_URL", "https://contra.com/community/for-you"),
#                     content=draft.content,
#                     status="live",
#                     note="Posted via Contra browser session",
#                 ))
#                 db.commit()
#             finally:
#                 db.close()
...
#         if result["status"] == "success":
#             update_article_draft(draft_id, status="posted", detail=result.get("detail"))
#             page_id = _extract_notion_page_id(result.get("url", ""))
#             db = SessionLocal()
#             try:
#                 db.add(PostedBacklink(
#                     platform="notion",
...
# #         if platform == "notion":
# #             prompt = f"""
# # Write a SHORT, crisp blog post (180-250 words) about: {topic}
# # This is for Gaper (gaper.io), to be published as a public Notion blog page.
...
# # IMPORTANT RULES:
# # - Line 1: a heading written as "## Headline text" (use the ## prefix), under 70 characters. No links, no brackets.
# # - Keep it skimmable: short paragraphs (2-3 sentences max), 1 more "## " heading further down if useful, and "* " bullets where useful.
# # - Do NOT mention gaper.io or any link anywhere in the article body - the link is added separately afterward.
# # - Genuinely useful, factual tone, not a sales pitch. No filler or repetition.
# # - Output ONLY the heading + article, nothing else.
# # """
# #             res = model.generate_content(prompt)
# #             text = (res.text or "").strip()

# #             # The Notion PAGE TITLE (top of page) is always rendered huge/bold
# #             # by Notion itself, no matter how short the text is - that can't
# #             # be styled smaller. So we deliberately never put the AI headline
# #             # there. The page Title is just the short topic string instead,
# #             # and the real headline lives in the body as a normal "## "
# #             # heading, which renders at normal article size.
# #             clean_title = topic.strip()[:60].rstrip(" -.,") or "Gaper Update"

# #             body = text
# #             if not body.lstrip().startswith("#"):
# #                 # Model didn't add a heading line - add one from the topic so
# #                 # the article still has a visible headline in the body.
# #                 body = f"## {clean_title}\n\n{body}"

# #             # Append the backlink as a marker line - notion_session_poster.py
# #             # turns [[LINK:anchor|url]] into a real clickable hyperlink with
# #             # clean anchor text instead of a raw pasted URL.
# #             body += "\n\n[[LINK:Learn more about Gaper|https://gaper.io]]"
...
# # src/article_studio.py - Update generate_draft for Notion
...
# This is for Gaper (gaper.io), to be published on Notion.

# Brand USPs:
# {usps}

# IMPORTANT - THIS IS A FRESH ARTICLE. WRITE SOMETHING NEW.
# Rules:
# - Line 1: Just a plain title. No links, no markdown, under 70 chars.
# - Then the body with clear paragraphs.
# - Use "## " for subheadings where helpful.
# - Use "* " for bullet points where helpful.
# - Keep it short and useful (300-400 words).
# - Do NOT mention gaper.io link in the article - it will be added separately.
# - Output ONLY the title + article, nothing else.
# """
...
#             # Add link marker - will be converted to clickable link by notion_session_poster
#             body += "\n\n[[LINK:Learn more about Gaper|https://gaper.io]]"
...
#     elif draft.platform == "notion":
#         # Types the draft into a new page in your real workspace (the saved
#         # browser session's own account/workspace) and leaves the browser
#         # window open. It does NOT click Publish and does NOT mark this as
#         # posted yet - that only happens once you confirm below, after you
#         # have actually reviewed and published it yourself in Notion.
#         result = post_to_notion_session(draft.title or draft.topic, draft.content)
...
#                 detail=f"NOTION_URL:{page_url}",
#             )
#             return {
#                 "status": "success",
#                 "detail": "Typed into your Notion workspace - browser stays open. Review it, hit Publish yourself, then click 'Confirm Published' on this draft.",
#                 "url": page_url,
#             }
#         else:
#             update_article_draft(draft_id, status="failed", detail=result.get("detail"))
...
#     """
#     Call this only after you've actually clicked Publish in the Notion
#     browser window yourself. This is what finally marks the draft as
#     posted and logs it to the Backlinks tab - submit_draft() only typed
#     it in and left it open for your review, it never auto-published.
#     """
#     draft = get_article_draft(draft_id)
#     if not draft:
#         return {"status": "failed", "detail": "Draft not found."}
#     if draft.platform != "notion":
#         return {"status": "failed", "detail": "Not a Notion draft."}
#     if draft.status != "awaiting_confirm":
#         return {"status": "failed", "detail": "This draft isn't waiting on a confirm - generate/submit it first."}

#     page_url = ""
#     if draft.detail and draft.detail.startswith("NOTION_URL:"):
#         page_url = draft.detail.split("NOTION_URL:", 1)[1].strip()

#     page_id = _extract_notion_page_id(page_url)
#     update_article_draft(draft_id, status="posted", detail=f"Published by you at {page_url}" if page_url else "Published by you.")
#     db = SessionLocal()
#     try:
#         db.add(PostedBacklink(
#             platform="notion",
...
#             content=f"{draft.title}\n\n{draft.content}",
#             status="live",
#             note="Posted via Notion browser session (manually confirmed).",
#             platform_post_id=page_id,
#         ))
#         db.commit()
#     finally:
#         db.close()
#     return {"status": "success", "detail": "Marked as posted and logged to Backlinks."}
...
# # src/article_studio.py - generate_notion_content function

# def generate_notion_content(topic: str) -> dict:
#     """Generate UNIQUE Notion article - FRESH every time"""
...
#     # ✅ YEH PROMPT HAR BAAR NAYA CONTENT DEGA
#     prompt = f"""
# Write a COMPLETELY NEW and UNIQUE blog article (500-700 words) about: {topic}
# This is for Gaper (gaper.io), to be published on Notion.
...
# IMPORTANT RULES:
# - This is a FRESH article - do NOT repeat any previous content
# - Use DIFFERENT examples, DIFFERENT phrasing each time
# - Line 1: ONLY the plain title text. NO links, NO markdown, under 70 characters.
# - Then a blank line, then the body.
# - Use "## " for headings, "* " for bullets, "**text**" for bold.
# - NO em dashes (—) - use commas or periods.
# - The link https://gaper.io must be in the BODY, NEVER in the title.
# - Genuinely useful, factual tone, not a sales pitch.
# - Output ONLY the title + article, nothing else.
# """
...
#         # Ensure link in body
#         if "https://gaper.io" not in body:
#             body += f"\n\nLearn more at https://gaper.io"
        
#         return {"title": clean_title, "body": body}
        
#     except Exception as e:
#         logger.error(f"Notion content generation failed: {e}")
#         return {
#             "title": topic[:70], 
#             "body": f"Content about {topic}. Learn more at https://gaper.io"
#         }
...
"""
Article Studio - generates and posts Gaper articles to Notion and Contra
using their saved browser sessions.
"""
...
# Import the working root-level Contra generator/poster directly
sys.path.insert(0, str(Path(__file__).parent.parent))
from post_to_contra import post_to_contra, generate_post_content
...
# Generation config applied to every Gemini call in this file - higher
# temperature/top_p so repeated topics don't produce near-identical
# articles each time.
...
# Paired with a random topic so the same topic doesn't produce the same
# article twice in a row - gives Gemini a different lens each time.
ANGLES = [
    "from a founder's perspective",
    "with a focus on common mistakes to avoid",
    "comparing before/after results",
    "as a practical step-by-step guide",
    "focusing on cost and time savings",
    "from a technical implementation view",
    "told through a hypothetical case study",
    "with a contrarian/unpopular-opinion angle",
]


def generate_auto_topic() -> str:
    """Picks a random topic + angle combo for variety across runs."""
...
def generate_draft(platform: str, topic: str = None, target_url: str = None) -> dict:
    """Generate a new draft. Auto-generates a topic+angle if not provided."""
    if not topic or topic.strip() == "":
        topic = generate_auto_topic()
        logger.info(f"Auto-generated topic: {topic}")
...
"""
            ...
            lines = text.split("\n")
            raw_title = lines[0].strip() if lines else topic[:70]
            clean_title = _clean_title(raw_title, fallback=topic[:70])

            body = "\n".join(lines[1:]).strip() if len(lines) > 1 else text
            # Link marker - notion_session_poster.py turns this into a real
            # clickable hyperlink instead of a raw pasted URL.
            body += "\n\n[[LINK:Learn more about Gaper|https://gaper.io]]"

            draft_id = create_article_draft(
                platform="notion", topic=topic, content=body, title=clean_title
            )
            return {"status": "success", "draft_id": draft_id, "title": clean_title, "content": body}
            ...
            contra_url = target_url or getattr(config, "CONTRA_FEED_URL", "https://contra.com/community/for-you")
            draft_id = create_article_draft(
                platform="contra", topic=topic, content=content,
                target_url=contra_url, title=topic
            )
            return {"status": "success", "draft_id": draft_id, "content": content, "target_url": contra_url}
...
def regenerate_draft(draft_id: int, feedback: str) -> dict:
    """Regenerate a draft based on human feedback from the QA loop."""
    draft = get_article_draft(draft_id)
    if not draft:
        return {"status": "failed", "detail": "Draft not found."}
...
def _extract_notion_page_id(url: str) -> str:
    """
    Pulls the 32-char page id off the end of a notion.so URL and formats
    it with dashes (8-4-4-4-12) the way the Notion API expects, so live
    edit/delete via the API can actually find the page.
    """
    if not url:
        return None
    m = re.search(r"([0-9a-fA-F]{32})(?:\?.*)?$", url)
...
def submit_draft(draft_id: int) -> dict:
    """
    Submits a draft live, using the same saved browser sessions Contra/
    Notion already use.

    - Contra: posts directly and immediately, then logs to Backlinks.
    - Notion: types the draft into your real workspace and leaves the
      browser open for YOUR review - it never auto-clicks Publish. Only
      confirm_notion_publish() (called after you publish it yourself)
      marks it posted and logs it to Backlinks.
    """
    draft = get_article_draft(draft_id)
    if not draft:
        return {"status": "failed", "detail": "Draft not found."}
    ...
    if draft.platform == "contra":
        ...
        if result["status"] == "success":
            ...
            return {
                ...
                "detail": "Typed into your Notion workspace - browser stays open. Review it, hit Publish yourself, then click 'Confirm Published' on this draft.",
                "url": page_url,
                "link_confirmed": result.get("link_confirmed", True),
            }
        else:
            update_article_draft(draft_id, status="failed", detail=result.get("detail"))
        return result
...
def confirm_notion_publish(draft_id: int) -> dict:
    ...
    Call this only after you've actually clicked Publish in the Notion
    browser window yourself. This is what finally marks the draft as
    posted and logs it to the Backlinks tab - submit_draft() only types
    it in and leaves it open for review, it never auto-publishes.
    """
    draft = get_article_draft(draft_id)
    if not draft:
        return {"status": "failed", "detail": "Draft not found."}
    ...
    page_id = _extract_notion_page_id(page_url)
    update_article_draft(
        draft_id, status="posted",
        detail=f"Published by you at {page_url}" if page_url else "Published by you."
    )
    db = SessionLocal()
    try:
        db.add(PostedBacklink(
...
def generate_notion_content(topic: str) -> dict:
    """
    Standalone generator used where a draft_id isn't needed yet - returns
    title/body directly instead of creating a draft row.
    """
    brand = get_brand_profile()
    usps = brand.get("usps", "")
