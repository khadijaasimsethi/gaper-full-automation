...
#     guidelines = (thread_data.get("guidelines") or "").lower()
...
def evaluate_seo_geo_aeo(text: str, is_ghost: bool = False) -> dict:
    ...
    if is_ghost:
        scores["seo"] = 95
        factual_density = len(re.findall(r'\d+', text))
        scores["geo"] = min(100, 70 + (factual_density * 5))
        has_lists = 15 if any(char in text for char in ['-', '*', '1.', '2.']) else 0
        scores["aeo"] = min(100, 75 + has_lists)
        return scores
...
def draft_reply(thread_data: dict) -> dict:
    ...
    Drafts a comment/reply for a thread (Block 5).
    Decides between standard citation mode or Ghost Mode based on site rules.
    Sets is_mock=True whenever Gemini wasn't actually used, so the dashboard
    and CLI can clearly warn a human before they approve fake placeholder
    content by mistake.
    """
    brand_profile = get_brand_profile()
    guidelines = (thread_data.get("guidelines") or "").lower()

    is_ghost = any(word in guidelines for word in [
        "strictly no links", "no promotional links", "advertising forbidden",
        "no external links", "links not allowed"
    ])
    if is_ghost:
        logger.info("Guidelines strictly forbid self-promotion. Activating GHOST MODE (no Gaper links).")
    else:
    ...
    if is_mock:
        reply_text = "[MOCK DRAFT - Gemini was unavailable, this is placeholder text, do not approve as-is]\n\n" + reply_text
