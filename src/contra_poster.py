...
def _type_content(page, post_box, content: str, delay: int = 20):
    """
    Type content into the composer.

    IMPORTANT: fill() sets the whole text in one atomic DOM action and
    does NOT fire the real per-keystroke input events that Contra's
    composer listens for. Contra's "auto-detect a URL and turn it into
    a live link" behavior only runs off those real keystroke events -
    so fill() leaves any https://... in the content as plain,
    non-clickable text.

    To keep links clickable, we always use real keystroke typing
    whenever the content contains a URL. fill() is only used as a
    faster path for link-free content, since it's still more robust
    against mid-type re-renders in that case.
    """
...
def post_to_contra_feed(content: str, target_url: str = None) -> dict:
    ...
    try:
        with sync_playwright() as p:
            ...
            _type_content(page, post_box, content, delay=20)
...
def post_reply_to_contra(target_url: str, content: str) -> dict:
    ...
    try:
        with sync_playwright() as p:
            ...
            _type_content(page, post_box, content, delay=25)
