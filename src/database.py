...
class ListingOpportunity(Base):
    ...
    posted_url = Column(String(500), nullable=True)  # the live listing page URL, once confirmed
...
class ArticleDraft(Base):
    ...
    content = Column(Text)
    is_ghost = Column(Boolean, default=False)  # True = community forbids links, no backlink included
...
class PostedBacklink(Base):
    ...
    is_ghost = Column(Boolean, default=False)  # True = no backlink was ever included (Ghost Mode) - skip 7-day link checks
    last_checked_at = Column(DateTime, nullable=True)  # when we last verified this link is still live
    link_status = Column(String(50), default='unchecked')  # unchecked, still_live, missing
    link_check_status = Column(String(50), default='unchecked')  # same as link_status - dashboard.py reads this exact name
    follow_up_due_at = Column(DateTime, default=lambda: datetime.datetime.utcnow() + datetime.timedelta(days=7))
    created_at = Column(DateTime, default=datetime.datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow)
...
def create_article_draft(platform: str, topic: str, content: str, target_url: str = None, title: str = None, is_ghost: bool = False) -> int:
    db = SessionLocal()
    try:
        draft = ArticleDraft(platform=platform, topic=topic, content=content,
                              target_url=target_url, title=title, status='draft', is_ghost=is_ghost)
        db.add(draft)
        db.commit()
        return draft.id
    finally:
...
# --- 7-day link health helpers ---

def get_backlinks_due_for_check(days: int = 7) -> list:
    """Live, non-ghost backlinks that are >= `days` old AND haven't been checked in the last `days`.
    Ghost posts are excluded entirely - they never included a Gaper link, so
    there's nothing to verify and checking them would just misfire a false
    'link missing' result."""
    db = SessionLocal()
    try:
        cutoff = datetime.datetime.utcnow() - datetime.timedelta(days=days)
        rows = db.query(PostedBacklink).filter(
            PostedBacklink.status == 'live',
            PostedBacklink.is_ghost == False,
        ).all()
        due = []
        for r in rows:
            if r.created_at and r.created_at > cutoff:
                continue  # too new, not due yet
            if r.last_checked_at and r.last_checked_at > cutoff:
                continue  # already checked recently
            due.append(r)
        return due
    finally:
        db.close()
...
def update_backlink_health(backlink_id: int, link_status: str):
    ...
    try:
        ...
        if row:
            ...
            row.link_check_status = link_status
